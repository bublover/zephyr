					  ...
					  struct in_addr *current_ip)
{
	...
	eth = net_ieee80211_fill_header(ctx, pkt, htons(NET_ETH_PTYPE_ARP),
				  NULL, NULL);
				...
				struct in_addr *current_ip)
{
	...
	if (!entry) {
		...
		if (!entry) {
			...
			NET_DBG("No pending, let's try to get a new entry.");
			entry = arp_entry_get_free();
			if (!entry) {
				/* Then let's take one from table? */
				NET_DBG("Then let's take one from table?");
		...
		if (!entry) {
			/* We cannot sen take one from table? the packet,
			 * the ARP cache is full or there is already
			 * a pending query to this IP address, so
			 * this packet must be discarded.
			 */
	...
	net_ieee80211_fill_header(ctx, pkt,
						...
						struct net_pkt *req)
{
	...
	net_ieee80211_fill_header(ctx, pkt, htons(NET_ETH_PTYPE_ARP),
			    net_if_get_link_addr(iface)->addr,
			    eth_query->src.addr);
...
enum net_verdict net_arp_input(struct net_pkt *pkt)
{
	...
	case NET_ARP_REQUEST:
		...
		NET_INFO("ARP request from %s [%s] for %s",
	...
	case NET_ARP_REPLY:
		NET_INFO("ARP reply from %s [%s] for %s",
			log_strdup(net_sprint_ipv4_addr(&arp_hdr->src_ipaddr)),
			log_strdup(net_sprint_ll_addr(
					   (u8_t *)&arp_hdr->src_hwaddr,
					   arp_hdr->hwlen)),
